...
! function(b) {
    ...
    b.scrollNav = {//顶部导航对象
        ...
        _getNavData(){
           ...
           getIndexData().then(function(data){
                ...
                let _html='',
                len = thi.navdata.length;
                for(let i=0;i<len;i++){
                  _html+='<li class="nav-bar-item" data-index='+thi.navdata[i].id+'>\
                          <span>'+thi.navdata[i].text+'</span>\
                      </li>'
                }
    ...
    b.slider={//幻灯片对象
        ...
        _initSlider(){
            ...
            this.slide = new BScroll.default('.slider',options)
        ...
        _getSlideData(){
            ...
            getIndexData().then(function(data){
                 ...
                 let _html='',
                 _dots='',
                 len = thi.slidata.length;
                for(let i=0;i<len;i++){
                        _html+='<div data-index='+thi.slidata[i].id+'>\
                                <a class="" href='+thi.slidata[i].href+'>\
                                <img src='+thi.slidata[i].src+' alt="">\
                            </a>\
                        </div>'
